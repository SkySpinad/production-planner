...
import { getListPropositionMock } from '../../services/serviceDB';
import ReactCountryFlag from 'react-country-flag';

export default function MenuTerritory2({ eventPropositionList, handleTerritoryPropositions, children }) {

  const [eventTerritory, setEventTerritory] = useState();
  const [eventPropositionListByTerritory, setEventPropositionListByTerritory] = useState([]);
  ...
  useEffect(() => {
    if(eventTerritory) {
      setEventPropositionListByTerritory(getListPropositionMock(eventTerritory))
    }
  }, [eventTerritory]);
...
function handleSelect(data) {
  ...
  <MenuMultiChoice title="Propositions" rows={eventPropositionListByTerritory} handleSelect={handleSelect} required style={{maxWidth: "335px", minWidth: "335px", marginTop:'10px'}} maxChildrenValue={children} disabled={eventTerritory ? false : true}/>
</HorizontalCentered>
